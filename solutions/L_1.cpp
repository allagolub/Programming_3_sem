...
public:
    std::vector<Node> st;
    std::vector<std::vector<int64_t>> result_index;
    void Insert(std::string& str, int64_t order_pattern) {
        ...
        st[curr_node_ind].index_pattern = order_pattern;
    ...
    void PrintEntries(int64_t ind, int64_t node_index) {
        ...
        while ((node_index != 0) && (node_index != -1)){
            result_index[st[node_index].index_pattern].push_back(ind - st[node_index].len_pattern + 2);
            node_index = st[node_index].exit_link;
        }
    }
};
void AhoCorasik(std::string& str, int64_t size_patterns) {
    ...
    for (int64_t i = 0; i < quantity_patterns; ++i) {
        std::cout << trie_for_aho_corasik.result_index[i].size() << ' ';
        int64_t quantity_curr_patter = trie_for_aho_corasik.result_index[i].size();
        for (int64_t j = 0; j < quantity_curr_patter; ++j) {
            std::cout << trie_for_aho_corasik.result_index[i][j] << ' ';
        }
        std::cout <<  '\n';
    }
}
